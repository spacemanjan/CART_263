...
// Constant defining ball's max speed
const FOOD_MAXSPEED = 10;
...
function setup() {
  ...
  positionFood();
  setInterval(changeFoodVelocity,1000);
  noCursor();
}
...
// updateFood()
//
// Updates the position of the food based on velocity
// constrains the food to the inside of the canvas
// sets a maximum speed at which the food can run
function updateFood() {
food.x += food.vx;
food.y += food.vy;
...
// changeFoodVelocity()
//
// Randomly changes the velocity of the food
// Is called by a setInterval function
function changeFoodVelocity(){
	food.vx = random(-FOOD_MAXSPEED,FOOD_MAXSPEED);
	food.vy = random(-FOOD_MAXSPEED,FOOD_MAXSPEED);
}
...
// Set the food's VX and VY as random based on the food's maxSpeed
function positionFood() {
  food.x = random(0,width);
  food.y = random(0,height);

  food.vx = random(0,FOOD_MAXSPEED);
  food.vy = random(0,FOOD_MAXSPEED);
}
